  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4">
        <div>
          <h2 className="text-xl md:text-3xl font-bold text-[var(--admin-text-main)] mb-4 md:mb-6 font-heading">
            Dashboard
          </h2>
        ...
        <div className="w-full overflow-x-auto pb-2 md:pb-0 md:w-auto scrollbar-hide">
          <div className="flex items-center gap-2 md:gap-3 w-max md:w-auto">
            <Link
              href="/admin/add-laptop"
              className="bg-[#0a2e5e] hover:bg-[#29abe2] text-white px-3 md:px-4 py-2 rounded-lg text-xs md:text-sm font-bold flex items-center gap-1.5 md:gap-2 transition-all whitespace-nowrap"
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
        ...
        <div className="bg-[var(--admin-card)] rounded-2xl p-4 md:p-6 border border-[var(--admin-border)] relative group overflow-hidden shadow-sm">
          ...
          <div className="text-2xl md:text-3xl font-bold text-[var(--admin-text-main)] mb-2 relative z-10 font-heading">
            {stats.totalProducts}
            <span className="text-sm font-normal text-[var(--admin-text-muted)] ml-1">
              Items
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        {/* Bar Chart Mockup */}
        <div className="lg:col-span-2 bg-[var(--admin-card)] rounded-2xl p-4 md:p-6 border border-[var(--admin-border)] shadow-sm">
          ...
          <div className="h-64 flex items-end justify-between gap-2 px-4">
            ...
            {chartData.sales.map((val, idx) => (
              <div
                key={idx}
                className="w-full flex flex-col justify-end items-center gap-2 group h-full"
              >
                <div className="w-full flex items-end justify-center h-[85%] relative gap-1">
                  <div
                    style={{ height: `${val}%` }}
                    className="w-3 md:w-6 bg-gray-200 rounded-t-md relative group-hover:bg-gray-300 transition-all"
                  ></div>
                  <div
                    style={{ height: `${chartData.sales?.[idx] * 0.6}%` }}
                    className="w-3 md:w-6 bg-[#0a2e5e] rounded-t-md relative group-hover:bg-blue-700 transition-all"
                  ></div>
                </div>
        ...
        {/* Donut Chart Mockup */}
        <div className="bg-[var(--admin-card)] rounded-2xl border border-[var(--admin-border)] p-4 md:p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-[var(--admin-text-main)]">
              Category Split
            </h3>
          ...
          <div className="relative h-48 mb-4 flex items-center justify-center">
            ...
            >
              <div className="w-32 h-32 bg-[var(--admin-card)] rounded-full absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center">
                <span className="text-2xl md:text-3xl font-bold text-[var(--admin-text-main)] font-heading">
                  {stats.totalProducts}
                </span>
                <span className="text-xs text-[var(--admin-text-muted)]">
                  Total Items
      ...
      {/* Recent Items Table */}
      <div className="bg-[var(--admin-card)] rounded-2xl p-4 md:p-6 border border-[var(--admin-border)] shadow-sm">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-lg font-bold text-[var(--admin-text-main)]">
            Recent Inventory
          </h3>
            ...
            <tbody className="text-sm">
              {recentProducts.length > 0 ? (
                recentProducts.map((product) => (
                  <tr
                    key={product._id}
                    className="border-b border-[var(--admin-border)] hover:bg-[var(--admin-hover)] transition-colors group"
                  >
                    ...
                    <td className="py-4 px-4 text-[var(--admin-text-muted)] capitalize whitespace-nowrap">
                      {product.category}
                    </td>
                    ...
                    <td className="py-4 px-4 text-[var(--admin-text-main)] font-medium">
                      {product.quantity}
                    </td>
                    <td className="py-4 pr-6 text-right text-[var(--admin-text-muted)] group-hover:text-[var(--admin-text-main)] cursor-pointer">
                      <Link
                        ...
                        className="inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-blue-50 hover:text-[#0a2e5e] transition-all"
                        title="Edit Product"
                      >
