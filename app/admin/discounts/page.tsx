  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-xl md:text-2xl font-bold text-[var(--admin-text-main)]">
          Discount Codes
        </h1>
        <button
          onClick={() => handleOpenModal()}
          className="bg-[#0a2e5e] text-white px-3 md:px-4 py-2 rounded-lg hover:bg-[#29abe2] transition shadow-lg shadow-blue-900/20 text-xs md:text-sm font-bold"
        >
          + Create New
        </button>
      </div>
      ...
      <div className="bg-[var(--admin-card)] rounded-xl border border-[var(--admin-border)] shadow-sm overflow-hidden">
        ...
        <div className="md:hidden divide-y divide-[var(--admin-border)]">
          {discounts.length === 0 ? (
            ...
            discounts.map((discount) => (
              <div key={discount._id} className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-bold text-sm text-[var(--admin-text-main)]">
                    {discount.code}
                  </span>
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] font-bold ${discount.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}
                  >
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="text-[#29abe2] text-xs font-medium"
                  >
        ...
        <div className="hidden md:block">
          <table className="w-full text-left text-sm text-[var(--admin-text-muted)]">
            <thead className="bg-[var(--admin-bg)] text-[var(--admin-text-main)] font-semibold border-b border-[var(--admin-border)]">
              <tr>
            ...
            <tbody className="divide-y divide-[var(--admin-border)]">
              {discounts.length === 0 ? (
                ...
                discounts.map((discount) => (
                  <tr
                    key={discount._id}
                    className="hover:bg-[var(--admin-hover)] transition"
                  >
                    <td className="p-4 font-bold text-[var(--admin-text-main)]">
                      {discount.code}
                    </td>
                    ...
                    <td className="p-4">
                      <span
                        className={`px-2 py-1 rounded text-xs font-bold ${discount.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700"}`}
                      >
                        {discount.isActive ? "Active" : "Inactive"}
                      </span>
                    </td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-[var(--admin-card)] rounded-2xl w-full max-w-lg p-4 md:p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg md:text-xl font-bold text-[var(--admin-text-main)] mb-4">
