  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/user-tags?search=${encodeURIComponent(search)}&page=${page}`,
      );
      if (res.ok) {
      ...
      <div className="bg-[var(--admin-card)] rounded-xl border border-[var(--admin-border)] shadow-sm">
        <div className="px-4 md:px-6 py-3 md:py-4 border-b border-[var(--admin-border)] flex items-center justify-between">
          <div>
            ...
            <p className="text-xs md:text-sm text-[var(--admin-text-muted)] mt-1">
              Manage user tags for targeted push notifications
            </p>
          </div>
        </div>
        ...
        {/* Tag Stats Overview */}
        {allTags.length > 0 && (
          <div className="px-4 md:px-6 py-3 md:py-4 border-b border-[var(--admin-border)] bg-[var(--admin-hover)]/50">
            <p className="text-xs font-bold text-[var(--admin-text-muted)] uppercase tracking-wider mb-3">
              Active Tags
            </p>
            <div className="flex flex-wrap gap-2">
              {allTags.map((t) => (
                <span
                  key={t.tag}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-[var(--admin-border)] rounded-full text-xs font-medium text-gray-700 shadow-sm"
                >
        ...
        {/* Search & Bulk Actions */}
        <div className="px-4 md:px-6 py-3 md:py-4 border-b border-[var(--admin-border)] flex flex-col gap-3 md:gap-4">
          <form
          ...
          >
            ...
            <button
              type="submit"
              className="px-4 py-2 bg-[#0a2e5e] text-white rounded-lg text-sm font-bold hover:bg-[#29abe2] transition-colors"
            >
              Search
            </button>
          </form>
          ...
          {selectedUsers.length > 0 && (
            <div className="flex flex-wrap items-center gap-2">
              ...
              <input
                ...
                className="bg-[var(--admin-hover)] border border-[var(--admin-border)] text-[var(--admin-text-main)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500 w-32 md:w-40"
            ...
            <>
              ...
              <div className="md:hidden divide-y divide-[var(--admin-border)]">
                {users.map((user) => (
                  <div
                    key={user._id}
                    className={`p-4 ${selectedUsers.includes(user._id) ? "bg-blue-50/50" : ""}`}
                  >
                      ...
                      <div className="flex-grow min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <div className="min-w-0">
                            <p className="font-semibold text-sm text-[var(--admin-text-main)] truncate">
                              {user.name}
                            </p>
                        ...
                        {user.location && (
                          <p className="text-[11px] text-[var(--admin-text-muted)] mb-1.5">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {user.tags && user.tags.length > 0 ? (
                            user.tags.map((tag) => (
                              <span
                                key={tag}
                                className="inline-flex items-center gap-0.5 px-2 py-0.5 bg-blue-50 border border-blue-200 rounded-full text-[10px] font-medium text-blue-700"
                              >
                                {tag}
                                <button
                                  onClick={() =>
                                    removeTagFromUser(user._id, tag)
                                  }
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-[var(--admin-hover)]/50 border-b border-[var(--admin-border)]">
                      ...
                      <th className="text-left px-4 py-3 text-xs font-bold text-[var(--admin-text-muted)] uppercase tracking-wider">
                        User
                      </th>
                      <th className="text-left px-4 py-3 text-xs font-bold text-[var(--admin-text-muted)] uppercase tracking-wider">
                        Location
                  ...
                  <tbody>
                    {users.map((user) => (
                      <tr
                        key={user._id}
                        className={`border-b border-[var(--admin-border)] hover:bg-[var(--admin-hover)]/50 transition-colors ${selectedUsers.includes(user._id) ? "bg-blue-50/50" : ""}`}
                      >
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {user.tags && user.tags.length > 0 ? (
                              user.tags.map((tag) => (
                                <span
                                  key={tag}
                                  className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-50 border border-blue-200 rounded-full text-[11px] font-medium text-blue-700 group"
                                >
                                  {tag}
                                  <button
                                    onClick={() =>
        ...
        {/* Pagination */}
        {totalPages > 1 && (
          <div className="px-4 md:px-6 py-3 md:py-4 border-t border-[var(--admin-border)] flex items-center justify-between">
            <p className="text-xs text-[var(--admin-text-muted)]">
