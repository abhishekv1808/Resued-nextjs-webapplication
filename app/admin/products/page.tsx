...
export default function AdminProducts() {
    ...
    const toggleStatus = async (product: any) => {
        try {
            const response = await fetch('/api/admin/products/toggle-status', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ productId: product._id, inStock: !product.inStock })
            });
    ...
    return (
        <div className="space-y-6">
            <header className="flex items-center justify-between sticky top-0 z-10 bg-[var(--admin-bg)]/95 backdrop-blur-sm py-4 transition-colors duration-300">
            ...
            <div className="bg-[var(--admin-card)] rounded-2xl border border-[var(--admin-border)] overflow-hidden shadow-xl">
                        ...
                        <tbody className="divide-y divide-[var(--admin-border)] text-sm">
                            {products.length > 0 ? (
                                products.map((product) => (
                                    <tr key={product._id} className="group hover:bg-[var(--admin-hover)] transition-colors duration-200">
                                        <td className="p-5">
                                            <div className="flex items-center gap-8">
                                                ...
                                                <div className="max-w-[350px]">
                                                    ...
                                                    <div className="flex flex-wrap gap-1">
                                                        {product.specifications && Object.values(product.specifications).slice(0, 3).map((spec: any, idx) => (
                                                            <span key={idx} className="px-1.5 py-0.5 rounded text-[10px] bg-gray-100 text-gray-600 border border-gray-200 truncate max-w-[100px]">{spec}</span>
                                                        ))}
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <td className="p-5 text-[var(--admin-text-main)]">
                                            ...
                                            <div className="text-xs text-[var(--admin-text-muted)] line-through">₹{(product.mrp).toLocaleString('en-IN')}</div>
                                        </td>
                                        <td className="p-5">
                                            ...
                                            <div className="mt-2">
                                                {product.quantity > 0 ? (
                                                    <span className="text-xs text-[var(--admin-text-muted)]">{product.quantity} units</span>
                                                ) : (
                                        ...
                                        <td className="p-5">
                                            <button
                                                ...
                                                title="Toggle Status"
                                            >
                                                <span className={`${product.inStock ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}></span>
                                            </button>
                                            <div className={`mt-1 text-[10px] font-bold ${product.inStock ? 'text-green-500' : 'text-[var(--admin-text-muted)]'}`}>
            ...
            {isModalOpen && selectedProduct && (
                <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                    <div className="flex min-h-screen items-center justify-center p-4 text-center sm:p-0">
                        ...
                        <div className="relative transform overflow-hidden rounded-2xl bg-[var(--admin-card)] text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-4xl">
                            {/* Header */}
                            ...
                            <div className="px-6 py-8">
                                <div className="flex flex-col lg:flex-row gap-8">
                                    ...
                                    <div className="lg:w-1/3">
                                        <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center justify-center aspect-square shadow-inner">
                                            {selectedProduct.images?.[0] || selectedProduct.image ? (
                                                <img src={selectedProduct.images?.[0] || selectedProduct.image} alt={selectedProduct.name} className="max-w-full max-h-full object-contain" />
                                            ) : (
                                                <span className="text-4xl text-gray-300 font-bold">{selectedProduct.name.charAt(0)}</span>
                                            )}
                                        </div>
                                        <div className="mt-4 flex justify-between items-center bg-[var(--admin-bg)] p-3 rounded-lg border border-[var(--admin-border)]">
                                            <span className="text-xs font-bold text-[var(--admin-text-muted)] uppercase tracking-wider">Stock Status</span>
                                            {selectedProduct.inStock ? (
                                                <span className="px-2 py-1 rounded text-xs font-bold bg-green-500/10 text-green-500 border border-green-500/20">In Stock ({selectedProduct.quantity} units)</span>
                                            ) : (
                                                <span className="px-2 py-1 rounded text-xs font-bold bg-red-500/10 text-red-500 border border-red-500/20">Out of Stock</span>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="lg:w-2/3 space-y-6">
                                        <div>
                                            <h2 className="text-2xl font-bold text-[var(--admin-text-main)] leading-tight mb-1">{selectedProduct.name}</h2>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="bg-[var(--admin-bg)] p-4 rounded-xl border border-[var(--admin-border)]">
                                                <span className="text-xs text-[var(--admin-text-muted)] uppercase tracking-wider block mb-1">Price</span>
                                                <div className="flex items-baseline gap-2">
                                                    <span className="text-xl font-bold text-[var(--admin-text-main)]">₹{selectedProduct.price.toLocaleString('en-IN')}</span>
                                                    <span className="text-sm text-[var(--admin-text-muted)] line-through">₹{selectedProduct.mrp.toLocaleString('en-IN')}</span>
                                                </div>
                                            </div>
                                            <div className="bg-[var(--admin-bg)] p-4 rounded-xl border border-[var(--admin-border)]">
                                                <span className="text-xs text-[var(--admin-text-muted)] uppercase tracking-wider block mb-1">Discount</span>
                                                <span className="text-xl font-bold text-green-500">{selectedProduct.discount}% OFF</span>
                                            </div>
                                        </div>

                                        <div>
                                            <h4 className="text-sm font-bold text-[var(--admin-text-main)] mb-2 uppercase tracking-wider">Description</h4>
                                        ...
                                        <div>
                                            <h4 className="text-sm font-bold text-[var(--admin-text-main)] mb-2 uppercase tracking-wider">Specifications</h4>
                                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                                {selectedProduct.specifications && Object.entries(selectedProduct.specifications).map(([key, value]: [string, any]) => (
                                                    <div key={key} className="bg-[var(--admin-bg)] p-3 rounded-lg border border-[var(--admin-border)]">
                                                        <span className="text-[10px] text-[var(--admin-text-muted)] uppercase tracking-wide block mb-0.5">{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                                                        <span className="font-bold text-[var(--admin-text-main)] text-xs truncate block" title={value}>{value}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
