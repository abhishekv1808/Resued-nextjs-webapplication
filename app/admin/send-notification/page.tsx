...
export default function SendNotificationPage() {
    ...
    const [userSearchResults, setUserSearchResults] = useState<{ _id: string; name: string; phone?: string; email?: string }[]>([]);
    ...
    const searchUsers = async (query: string) => {
        ...
        try {
            const res = await fetch(`/api/admin/notification-targets?searchUser=${encodeURIComponent(query)}`);
            if (res.ok) {
                const data = await res.json();
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (audience === 'by_tags') {
            const checkedTags = Array.from(e.currentTarget.querySelectorAll<HTMLInputElement>('input[name="tag_checkbox"]:checked'))
                .map(cb => cb.value);
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-[var(--admin-card)] rounded-xl border border-[var(--admin-border)] p-6 shadow-sm">
                            <h3 className="text-lg font-bold text-[var(--admin-text-main)] mb-4 flex items-center gap-2">
                                <i className="ri-notification-3-line text-[#0a2e5e]"></i> Notification Details
                            </h3>
                            <div className="grid grid-cols-1 gap-6">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <select
                                            name="audience"
                                            value={audience}
                                            onChange={(e) => { setAudience(e.target.value); setSelectedUserId(''); setSelectedUserName(''); }}
                                ...
                                {audience === 'specific_user' && (
                                    <div>
                                        <label className="block text-xs font-bold text-[var(--admin-text-muted)] uppercase tracking-wider mb-2">Search User</label>
                                        <div className="relative">
                                            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => { setSelectedUserName(''); searchUsers(e.target.value); }}
                                        ...
                                        {selectedUserId && (
                                            <div className="mt-2 flex items-center gap-2 text-sm text-green-600 bg-green-50 px-3 py-2 rounded-lg border border-green-200">
                                                ...
                                                <button type="button" onClick={() => { setSelectedUserId(''); setSelectedUserName(''); setUserSearchQuery(''); }} className="ml-auto text-gray-400 hover:text-red-500">
                                            ...
                                            <div className="mt-2 bg-white border border-[var(--admin-border)] rounded-lg shadow-lg max-h-48 overflow-y-auto">
                                                {userSearchResults.map(u => (
                                                    <button
                                                        key={u._id}
                                                        type="button"
                                                        onClick={() => { setSelectedUserId(u._id); setSelectedUserName(u.name + (u.phone ? ` (${u.phone})` : '')); setUserSearchResults([]); }}
                                                    ...
                                                    >
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="text-xs text-gray-500">{u.phone || u.email || 'No contact info'}</div>
                                                        </div>
                                                    </button>
                                                ))}
                                            </div>
                                ...
                                {audience === 'by_tags' && (
                                    <div>
                                        <label className="block text-xs font-bold text-[var(--admin-text-muted)] uppercase tracking-wider mb-2">Select Tags</label>
                                        {availableTags.length > 0 ? (
                                            <div className="flex flex-wrap gap-2">
                                                {availableTags.map(tag => (
                                                    <label key={tag} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[var(--admin-hover)] border border-[var(--admin-border)] rounded-full text-sm cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors has-[:checked]:bg-blue-100 has-[:checked]:border-blue-400 has-[:checked]:text-blue-700">
                                                        <input type="checkbox" name="tag_checkbox" value={tag} className="w-3.5 h-3.5 rounded accent-blue-600" />
                                                        <span className="font-medium">{tag}</span>
                                                    </label>
                                                ))}
                                            </div>
                                        ) : (
                                            <p className="text-sm text-[var(--admin-text-muted)] italic">No tags found. Add tags to users first.</p>
                                        )}
                                ...
                                {audience === 'by_location' && (
                                    <div>
                                        <label className="block text-xs font-bold text-[var(--admin-text-muted)] uppercase tracking-wider mb-2">Location</label>
                                        <div className="relative">
                                            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            ...
                                            {availableLocations.length > 0 ? (
                                                <>
                                                    <select name="location" className="w-full pl-10 bg-[var(--admin-hover)] border border-[var(--admin-border)] text-[var(--admin-text-main)] rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors appearance-none">
                                                ...
                                                <input type="text" name="location" className="w-full pl-10 bg-[var(--admin-hover)] border border-[var(--admin-border)] text-[var(--admin-text-main)] rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" placeholder="e.g. Bangalore" />
                                            )}
                                        </div>
                                    </div>
                                )}
                                ...
                                {audience === 'inactive_users' && (
                                    <div>
                                        <label className="block text-xs font-bold text-[var(--admin-text-muted)] uppercase tracking-wider mb-2">Inactive For (Days)</label>
                                        <div className="relative">
                                            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                                <i className="ri-time-line text-gray-400"></i>
                                            </div>
                                            <input type="number" name="inactiveDays" defaultValue={7} min={1} max={365} className="w-full pl-10 bg-[var(--admin-hover)] border border-[var(--admin-border)] text-[var(--admin-text-main)] rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" />
                                        </div>
                                        <p className="text-xs text-[var(--admin-text-muted)] mt-1">Users who haven&apos;t logged in for this many days</p>
                                    </div>
                                )}
                                ...
                                <div className="flex items-start gap-2 text-xs text-[var(--admin-text-muted)] bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">
                                    ...
                                    <span>
                                        {audience === 'all' && 'Sends to every device that subscribed to push notifications (including non-logged-in visitors).'}
                                        {audience === 'registered_users' && 'Sends only to users who have an account and are subscribed to push.'}
                                        {audience === 'specific_user' && 'Sends to a single user\'s subscribed device(s). Search and select a user above.'}
                                        {audience === 'cart_abandonment' && 'Sends to users who currently have items in their cart but haven\'t checked out.'}
                                        {audience === 'by_tags' && 'Sends to users matching any of the selected tags. Tags are assigned automatically on purchase or manually by admin.'}
                                        {audience === 'by_location' && 'Sends to users whose saved location matches the selected city/area.'}
                                        {audience === 'inactive_users' && 'Sends to users who haven\'t logged in for the specified number of days. Great for re-engagement.'}
                                        {audience === 'purchase_history' && 'Sends to all users who have made at least one successful purchase.'}
                                    </span>
                                </div>
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="text" name="title" className="w-full pl-10 bg-[var(--admin-hover)] border border-[var(--admin-border)] text-[var(--admin-text-main)] rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-[#29abe2] focus:ring-1 focus:ring-[#29abe2] transition-colors" placeholder="e.g. Big Sale Starting Now!" required />
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-[var(--admin-text-muted)] uppercase tracking-wider mb-2">Message Body</label>
                                    <div className="relative">
                                        <textarea name="body" rows={4} className="w-full bg-[var(--admin-hover)] border border-[var(--admin-border)] text-[var(--admin-text-main)] rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors resize-none" placeholder="e.g. Get 50% off on all Laptops. Limited time offer." required></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="text" name="url" className="w-full pl-10 bg-[var(--admin-hover)] border border-[var(--admin-border)] text-[var(--admin-text-main)] rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-[#29abe2] focus:ring-1 focus:ring-[#29abe2] transition-colors" placeholder="e.g. /laptops or https://reused.in/offers" />
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="p-4 bg-[var(--admin-hover)]/30 rounded-xl border border-[var(--admin-border)]">
                                        ...
                                        <div className="space-y-3">
                                            <div>
                                                ...
                                                <input type="text" name="action1_title" className="w-full bg-[var(--admin-bg)] border border-[var(--admin-border)] text-[var(--admin-text-main)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="e.g. View Offer" />
                                            </div>
                                            <div>
                                                <label className="block text-xs text-[var(--admin-text-muted)] mb-1">Action URL</label>
                                                <input type="text" name="action1_url" className="w-full bg-[var(--admin-bg)] border border-[var(--admin-border)] text-[var(--admin-text-main)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="e.g. /offers" />
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-4 bg-[var(--admin-hover)]/30 rounded-xl border border-[var(--admin-border)]">
                                        ...
                                        <div className="space-y-3">
                                            <div>
                                                <label className="block text-xs text-[var(--admin-text-muted)] mb-1">Button Title</label>
                                                <input type="text" name="action2_title" className="w-full bg-[var(--admin-bg)] border border-[var(--admin-border)] text-[var(--admin-text-main)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="e.g. Call Now" />
                                            </div>
                                            <div>
                                                <label className="block text-xs text-[var(--admin-text-muted)] mb-1">Action URL</label>
                                                <input type="text" name="action2_url" className="w-full bg-[var(--admin-bg)] border border-[var(--admin-border)] text-[var(--admin-text-main)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="e.g. tel:+1234567890" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
