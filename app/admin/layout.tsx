...
export default function AdminLayout({
    ...
    return (
      <>
        ...
        <div className="h-16 flex items-center px-4 border-b border-[var(--admin-border)] flex-shrink-0">
          <div
            className={`flex items-center transition-all duration-300 ${isOpen ? "opacity-100 w-auto" : "opacity-0 w-0 overflow-hidden"}`}
          >
            <Image
          ...
          {isMobile ? (
            <button
              ...
              className="ml-auto w-8 h-8 rounded-lg flex items-center justify-center text-[var(--admin-text-muted)] hover:text-[var(--admin-text-main)] hover:bg-[var(--admin-hover)] transition-all"
            >
            ...
            <button
              ...
              className={`w-8 h-8 rounded-lg flex items-center justify-center text-[var(--admin-text-muted)] hover:text-[var(--admin-text-main)] hover:bg-[var(--admin-hover)] transition-all ${!isOpen ? "mx-auto" : "ml-auto"}`}
            >
              <i
                className={`text-lg ${isOpen ? "ri-menu-fold-line" : "ri-menu-unfold-line"}`}
              ></i>
            </button>
          )}
        </div>
        ...
        <div className="flex-grow px-3 py-3 space-y-4 overflow-y-auto scrollbar-hide">
          ...
          <div>
            <p
              className={`px-3 text-[10px] font-bold text-[var(--admin-text-muted)] uppercase tracking-widest mb-2 whitespace-nowrap transition-all duration-300 overflow-hidden ${isOpen ? "opacity-60" : "opacity-0 w-0"}`}
            >
              Main Menu
            </p>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`flex items-center px-3 py-2.5 rounded-lg transition-all group whitespace-nowrap ${
                      isActive
                  ...
                  >
                    <i
                      className={`${item.icon} text-[17px] flex-shrink-0 ${isOpen ? "mr-3" : "mx-auto"} ${isActive ? "text-[#29abe2]" : "text-[var(--admin-text-muted)] group-hover:text-[#29abe2]"}`}
                    ></i>
                    <span
                      className={`font-medium text-[13px] transition-all duration-300 overflow-hidden ${isOpen ? "opacity-100 w-auto" : "opacity-0 w-0"}`}
                    >
                      {item.name}
                    </span>
                    {isActive && (
                      <div
                        className={`w-1.5 h-1.5 rounded-full bg-[#29abe2] flex-shrink-0 transition-all duration-300 ${isOpen ? "ml-auto" : "hidden"}`}
                      ></div>
                    )}
                  </Link>
                );
        ...
        <div className="flex-shrink-0 border-t border-[var(--admin-border)] p-3">
          ...
          <Link
            href="/"
            className={`flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-[var(--admin-hover)] transition-colors group mb-2 ${!isOpen ? "justify-center" : ""}`}
            title={!isOpen ? "View Store" : ""}
          >
            <i className="ri-external-link-line text-[17px] text-[var(--admin-text-muted)] group-hover:text-[#29abe2] flex-shrink-0"></i>
            <span
              className={`font-medium text-[13px] text-[var(--admin-text-muted)] group-hover:text-[var(--admin-text-main)] transition-all duration-300 overflow-hidden ${isOpen ? "opacity-100 w-auto" : "opacity-0 w-0"}`}
            >
              View Store
            </span>
          </Link>
          ...
          >
            <div
              className={`flex items-center gap-2.5 flex-grow min-w-0 px-2 py-2 rounded-lg ${!isOpen ? "justify-center px-0" : ""}`}
            >
              ...
              <div
                className={`flex-grow min-w-0 transition-all duration-300 overflow-hidden ${isOpen ? "opacity-100 w-auto" : "opacity-0 w-0"}`}
              >
                <h4 className="text-[var(--admin-text-main)] text-[13px] font-bold truncate leading-tight">
                  Admin User
                </h4>
                <p className="text-[11px] text-[var(--admin-text-muted)] leading-tight">
                  Administrator
                </p>
              </div>
            </div>
            <button
              ...
              className={`flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg bg-red-50 hover:bg-red-100 text-red-500 hover:text-red-600 transition-all ${!isOpen ? "" : ""}`}
              title="Logout"
            >
  ...
  return (
    <div className="flex min-h-screen bg-[var(--admin-bg)] text-[var(--admin-text-main)] font-sans antialiased overflow-hidden selection:bg-[#29abe2] selection:text-white transition-colors duration-300">
      ...
      <nav
        className={`hidden lg:flex bg-[var(--admin-card)] border-r border-[var(--admin-border)] h-screen text-[var(--admin-text-muted)] flex-col flex-shrink-0 relative z-30 transition-all duration-300 ease-in-out overflow-hidden ${isSidebarOpen ? "w-60" : "w-[68px]"}`}
      >
      ...
      {isMobileMenuOpen && (
        <>
          ...
          ></div>
          {/* Drawer */}
          <nav className="fixed left-0 top-0 h-full w-72 bg-[var(--admin-card)] border-r border-[var(--admin-border)] text-[var(--admin-text-muted)] flex flex-col z-50 lg:hidden animate-slide-in-left shadow-2xl">
      ...
      <main className="flex-grow h-screen overflow-y-auto w-full relative flex flex-col">
        {/* Header */}
        <header className="flex items-center justify-between px-4 md:px-6 lg:px-8 py-4 lg:py-6 sticky top-0 z-20 bg-[var(--admin-bg)]/95 backdrop-blur-sm transition-colors duration-300 gap-3">
          {/* Mobile hamburger */}
          <button
            onClick={() => setIsMobileMenuOpen(true)}
            className="lg:hidden w-10 h-10 rounded-xl flex items-center justify-center text-[var(--admin-text-muted)] hover:text-[var(--admin-text-main)] hover:bg-[var(--admin-hover)] transition-all flex-shrink-0"
          >
            <i className="ri-menu-line text-xl"></i>
          </button>

          {/* Search */}
          <div className="relative flex-grow max-w-96">
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            <div className="flex items-center gap-1 sm:gap-2 bg-[var(--admin-card)] border border-[var(--admin-border)] rounded-full px-1.5 sm:px-2 py-1">
              ...
              <button className="w-8 h-8 rounded-full hover:bg-[var(--admin-hover)] flex items-center justify-center text-[var(--admin-text-muted)] transition-colors hidden sm:flex">
        ...
        {/* Page Content */}
        <div className="flex-grow px-4 md:px-6 lg:px-8 pb-6 lg:pb-8">
          {children}
        </div>
      </main>
                    ...
                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                      ...
                      <div className="mt-2">
                        <p className="text-sm text-gray-500">
                          ...
                          login again to access the admin dashboard.
                        </p>
                      </div>
                    </div>
                  </div>
