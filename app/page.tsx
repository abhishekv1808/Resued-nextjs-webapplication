...
export const metadata: Metadata = {
  ...
  description:
    "Looking for low price laptops in Bangalore? Get best certified refurbished laptops under 25k, second hand monitors, and used desktops at Reused.in. Visit us in Jayanagar.",
  alternates: {
    canonical: "https://reused.in",
  },
};
...
async function getHomeData() {
  ...
  const [
    ...
    Product.find({ category: "laptop", brand: "Apple" })
      .select(productProjection)
      .limit(8)
      .lean(),
    Product.find({ category: "laptop", brand: "Dell" })
      .select(productProjection)
      .limit(8)
      .lean(),
    Product.find({ category: "laptop", brand: "HP" })
      .select(productProjection)
      .limit(8)
      .lean(),
    Product.find({ category: "laptop", brand: "Lenovo" })
      .select(productProjection)
      .limit(8)
      .lean(),
    Product.find()
      ...
      .limit(4)
      .lean(),
    Product.find({ category: "desktop" })
      .select(productProjection)
      .limit(8)
      .lean(),
    Product.find({ category: "monitor" })
      .select(productProjection)
      .limit(8)
      .lean(),
    Product.find({ category: "accessory" })
      .select(productProjection)
      .limit(8)
      .lean(),
  ...
  return (
    <>
      ...
      <main>
        ...
        <section className="bg-white">
          <div className="max-w-6xl mx-auto px-4 py-4 md:py-8 space-y-4 md:space-y-8">
            <h1 className="sr-only">
              Reused - Used Laptops & Desktops Bangalore
            </h1>
            <HeroCarousel />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
              <div className="bg-blue-50 rounded-2xl p-3 md:p-8 flex flex-row items-center justify-between relative overflow-hidden group hover:shadow-md transition-all h-[120px] md:h-auto border border-blue-100/50">
                <div className="w-7/12 md:w-1/2 z-10">
                  <span className="text-[#0a2e5e] font-bold text-[10px] md:text-sm mb-1 md:mb-2 block uppercase tracking-wider">
                    Power & Performance
                  </span>
                  <h3 className="text-base md:text-2xl font-bold text-gray-900 mb-0.5 md:mb-2 leading-tight">
                    Build Your Dream Workstation
                  </h3>
                  <Link
                    href="/desktops"
                    className="inline-flex items-center text-[#29abe2] font-bold text-xs md:text-base mt-2 md:mt-4 group-hover:underline"
                  >
                ...
                <div className="w-5/12 md:w-1/2 mt-0 md:mt-0 z-10 flex justify-end h-full items-center">
                  <Image
                    src="/images/lenovo-hero-image.png"
                    alt="Desktop"
                    width={200}
                    height={200}
                    className="h-full md:h-48 object-contain drop-shadow-xl group-hover:scale-110 transition-transform"
                  />
                </div>
              </div>
              ...
              <div className="bg-[#eefcfc] rounded-2xl p-3 md:p-8 flex flex-row items-center justify-between relative overflow-hidden group hover:shadow-md transition-all h-[120px] md:h-auto">
                <div className="w-7/12 md:w-1/2 z-10">
                  <span className="text-teal-500 font-semibold text-[10px] md:text-sm mb-1 md:mb-2 block">
                    Visual Excellence
                  </span>
                  <h3 className="text-base md:text-2xl font-bold text-gray-900 mb-0.5 md:mb-2 leading-tight">
                    4K & High Refresh Rate Monitors
                  </h3>
                  <Link
                    href="/monitors"
                    className="inline-flex items-center text-teal-600 font-semibold text-xs md:text-base mt-2 md:mt-4 group-hover:underline"
                  >
                    Order Now <i className="ri-arrow-right-line ml-1"></i>
                  </Link>
                </div>
                <div className="w-5/12 md:w-1/2 mt-0 md:mt-0 z-10 flex justify-end h-full items-center">
                  <Image
                    src="/images/dell-monitor-hero-image.png"
                    alt="Monitor"
                    width={200}
                    height={200}
                    className="h-full md:h-48 object-contain drop-shadow-xl group-hover:scale-110 transition-transform"
        ...
        <section
          id="why-choose-us"
          className="bg-white py-8 md:py-16 border-t border-gray-100 rounded-2xl mb-0 relative overflow-hidden"
        >
          <div className="max-w-6xl mx-auto px-4">
            <div className="text-center mb-6 md:mb-12">
              <h2 className="text-xl md:text-3xl font-bold text-gray-900 mb-2 px-4 md:px-0">
                Why Choose Reused?
              </h2>
              <p className="text-gray-500 text-xs md:text-base">
                The trusted destination for premium used technology.
              </p>
            </div>
              ...
              {[
                {
                  ...
                  title: "70%",
                  subtitle: "Box Savings",
                  desc: "Get premium tech for less.",
                },
                {
                ...
                <div
                  key={idx}
                  className="text-center group p-3 md:p-6 rounded-2xl hover:bg-blue-50 transition-colors duration-300 border border-transparent hover:border-blue-100"
                >
                  <div className="w-10 h-10 md:w-16 md:h-16 mx-auto bg-blue-50 rounded-full flex items-center justify-center mb-2 md:mb-4 group-hover:bg-[#0a2e5e] transition-colors duration-300">
                    <i
                      className={`${item.icon} text-lg md:text-2xl text-[#0a2e5e] group-hover:text-white transition-colors duration-300`}
                    ></i>
                  </div>
                  <h3 className="text-lg md:text-3xl font-bold text-gray-900 mb-1 group-hover:scale-110 transition-transform duration-300 flex items-center justify-center gap-1">
                    ...
                    {item.subIcon && (
                      <i
                        className={`${item.subIcon} text-yellow-400 text-base md:text-xl`}
                      ></i>
                    )}
                  </h3>
                  <p className="text-gray-900 font-semibold text-[10px] md:text-sm uppercase tracking-wide mb-1">
                    {item.subtitle}
                  </p>
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-8 gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1 md:mb-2">
                    ...
                    <span className="text-[#0a2e5e] font-bold text-[10px] md:text-sm uppercase tracking-wider">
                      Don&apos;t Miss Out
                    </span>
                  </div>
        ...
        {[
          {
            title: "Used Apple Laptops",
            brand: "Apple",
            logo: "/images/apple-logo.png",
            data: appleLaptops,
            desc: "Premium Apple MacBooks with M-series chips.",
          },
          {
            title: "Used Dell Laptops",
            brand: "Dell",
            logo: "/images/dell-logo.png",
            data: dellLaptops,
            desc: "Reliable Dell XPS and Latitude.",
          },
          {
            title: "Used HP Laptops",
            brand: "HP",
            logo: "/images/hp-logo.png",
            data: hpLaptops,
            desc: "Versatile HP Spectre and Envy.",
          },
          {
            title: "Used Lenovo Laptops",
            brand: "Lenovo",
            logo: "/images/lenovo-logo.png",
            data: lenovoLaptops,
            desc: "Business-class ThinkPads and IdeaPads.",
          },
        ].map((section, idx) => (
          <ProductRow
            key={idx}
