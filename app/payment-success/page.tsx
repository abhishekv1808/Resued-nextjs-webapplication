...
function PaymentSuccessContent() {
    ...
    const fetchOrder = async () => {
        try {
            ...
            const found = allOrders.find((o: any) =>
                o._id === orderId ||
                o.phonePeMerchantTransactionId === merchantOrderId
            );
    ...
    const subtotal = order?.products?.reduce((acc: number, item: any) => acc + ((item.product?.price || 0) * item.quantity), 0) || 0;
            ...
            <main className="min-h-screen bg-gray-50 relative overflow-hidden">
                        ...
                        {Array.from({ length: 40 }).map((_, i) => (
                            <div
                                ...
                                style={{
                                    ...
                                    backgroundColor: ['#0a2e5e', '#29abe2', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899'][Math.floor(Math.random() * 7)],
                ...
                <div className="max-w-2xl mx-auto px-4 py-8 md:py-12">
                    ...
                    <div className="text-center mb-6">
                        <div className={`relative w-20 h-20 mx-auto mb-4 ${!isPending ? 'animate-success-pop' : ''}`}>
                            <div className={`absolute inset-0 rounded-full ${isPending ? 'bg-amber-100' : 'bg-green-100'} animate-ping-slow opacity-40`}></div>
                            <div className={`relative w-20 h-20 rounded-full flex items-center justify-center ${isPending ? 'bg-amber-500' : 'bg-emerald-500'} shadow-lg`}>
                                <svg
                                    className="w-12 h-12"
                                    fill="none"
                                    viewBox="0 0 24 24"
                    ...
                    <div className="bg-white rounded-2xl shadow-xl shadow-gray-200/60 border border-gray-100 overflow-hidden">
                        ...
                        <div className="bg-[#0a2e5e] px-6 py-4 flex items-center justify-between">
                            <div>
                                <p className="text-[10px] font-bold text-blue-300 uppercase tracking-widest">Order ID</p>
                                <p className="text-white font-mono text-sm font-bold mt-0.5">
                                    {order?.phonePeMerchantTransactionId || order?._id || merchantOrderId || '—'}
                                </p>
                            </div>
                        ...
                        {order ? (
                            <div className="p-5 md:p-6">
                                ...
                                <div className="mb-5">
                                    <h3 className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                                        <i className="ri-shopping-bag-3-line text-sm"></i>Items Ordered
                                    </h3>
                                ...
                                <div className="space-y-2.5 mb-4">
                                    ...
                                    {discount > 0 && (
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className="text-green-600 font-medium tabular-nums">-{formatCurrency(discount)}</span>
                                        </div>
                                    )}
                                    <div className="flex justify-between text-sm">
                                            ...
                                            <div>
                                                ...
                                                {order.customerPhone && (
                                                    <p className="text-xs text-gray-400 mt-1 flex items-center gap-1">
                                                        <i className="ri-phone-line"></i>{order.customerPhone}
                                                    </p>
                                                )}
                                            </div>
                                        </div>
                                ...
                                {order.phonePePaymentId && (
                                    <div className="mt-4 flex items-center gap-2 bg-blue-50/60 rounded-lg px-4 py-2.5 border border-blue-100">
                                        ...
                                        <div className="min-w-0">
                                            <p className="text-[10px] font-bold text-blue-400 uppercase tracking-wider">Payment Reference</p>
                                            <p className="text-xs font-mono text-blue-700 truncate">{order.phonePePaymentId}</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="text-center py-10 px-6">
                                ...
                                {merchantOrderId && (
                                    <p className="text-xs font-mono text-gray-400">Reference: {merchantOrderId}</p>
                                )}
                            </div>
                        )}
                    </div>
                    ...
                    <p className="text-center text-xs text-gray-400 mt-5">
                        Need help? <Link href="/contact-us" className="text-[#0a2e5e] font-semibold hover:underline">Contact Support</Link>
                    </p>
                </div>
            </main>
            <Footer />
