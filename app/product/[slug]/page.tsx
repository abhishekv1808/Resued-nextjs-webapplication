...
export async function generateMetadata({
  ...
  const product = await Product.findOne({ slug })
    .select("name description image brand category")
    .lean();
  ...
  const description =
    ...
    `Buy ${product.name} at Reused. Best price for ${product.brand} ${product.category}.`;
        ...
        <div className="max-w-7xl mx-auto px-3 md:px-4">
          ...
          <nav className="flex mb-3 md:mb-6 text-xs md:text-sm text-gray-500">
            ...
            <Link
              ...
              className="hover:text-[#0a2e5e] capitalize"
            >
          ...
          <div className="mt-8 md:mt-16">
            <div className="flex justify-between items-end mb-4 md:mb-8">
              ...
              <Link
                href={`/${product.category}s`}
                className="text-[#0a2e5e] font-semibold hover:underline flex items-center gap-1 text-xs md:text-sm"
              >
