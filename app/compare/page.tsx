...
export default function ComparePage() {
  const { selectedProductIds, setSelectedProductIds, clearCompare } =
    useCompare();
  ...
  useEffect(() => {
    if (
      ...
      const products = selectedProductIds.map(
        (id) => allProducts.find((p) => p._id === id) || null,
      );
      setSelectedProducts([
        products[0] || null,
        products[1] || null,
  ...
  return (
    <>
      <style jsx global>{`
                ...
                select.compare-dropdown option {
                    padding: 10px 8px;
                    font-size: 0.875rem;
                    line-height: 1.4;
                ...
                select.compare-dropdown option:hover {
                    background-color: #fee2e2;
                }
                
                select.compare-dropdown option:disabled {
                    color: #9ca3af;
                    background-color: #f3f4f6;
                }
                
                select.compare-dropdown optgroup {
                    font-weight: 600;
                    font-size: 0.875rem;
                    padding: 8px 4px;
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-gray-500">
                Select up to 3 products to compare specifications and prices
              </p>
            </div>
            {hasAnySelected && (
              <button
              ...
              {[0, 1, 2].map((index) => (
                <div key={index} className="relative">
                  ...
                  <select
                    ...
                    className="compare-dropdown w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 bg-white text-gray-900 text-sm hover:border-gray-400 transition-colors cursor-pointer"
                    style={{
                      maxWidth: "100%",
                      overflow: "hidden",
                  ...
                  >
                    ...
                    {laptops.length > 0 && (
                      <optgroup label="💻 Laptops" className="font-semibold">
                        {laptops.map((product) => (
                          <option
                            key={product._id}
                            value={product._id}
                    ...
                    {desktops.length > 0 && (
                      <optgroup label="🖥️ Desktops" className="font-semibold">
                    ...
                    {monitors.length > 0 && (
                      <optgroup label="🖥️ Monitors" className="font-semibold">
          ...
          {hasAnySelected ? (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="p-4 text-left font-bold text-gray-900 bg-gray-50 sticky left-0 z-10 min-w-[150px]">
                      Specification
                    </th>
                    {selectedProducts.map((product, index) => (
                      <th
                        key={index}
                        className="p-4 text-center bg-gray-50 min-w-[250px]"
                      >
                        {product ? (
                ...
                <tbody>
                  ...
                  <tr className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    {selectedProducts.map((product, index) => (
                      <td
                        key={index}
                        className="p-4 text-center text-gray-900 capitalize"
                      >
                        {product?.category || "-"}
                      </td>
                    ))}
                  ...
                  {specKeys.map((key) => (
                    ...
                    >
                      <td className="p-4 font-semibold text-gray-700 bg-gray-50 sticky left-0 z-10 capitalize">
                        {key}
                      </td>
                      {selectedProducts.map((product, index) => (
                        <td
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-12 text-center">
              ...
              <p className="text-gray-500 mb-6">
                Select products from the dropdowns above to start comparing.
              </p>
            </div>
          )}
        </div>
