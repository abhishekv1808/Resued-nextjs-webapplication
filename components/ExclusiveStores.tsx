                ...
                <div className="flex flex-col md:flex-row justify-between items-end mb-6 md:mb-8 gap-4">
                    <div>
                        ...
                        <div className="flex gap-3 md:gap-4">
                            <div className="bg-red-50 text-gray-800 px-3 py-1.5 md:px-4 md:py-2 rounded-full flex items-center gap-2 text-xs md:text-sm font-medium">
                                <i className="ri-map-pin-line text-[#a51c30]"></i> 3 Experience Centres
                            </div>
                            <div className="bg-red-50 text-gray-800 px-3 py-1.5 md:px-4 md:py-2 rounded-full flex items-center gap-2 text-xs md:text-sm font-medium">
                                <i className="ri-star-fill text-[#a51c30]"></i> 4.8+ Star Ratings
                            </div>
                        </div>
                    </div>

                    <div className="flex flex-col items-end gap-3 md:gap-4 w-full md:w-auto">
                        <Link href="#" className="text-[#a51c30] font-semibold hover:underline text-sm hidden md:block">View all stores</Link>
                        <div className="relative w-full md:w-80">
                            <input type="text" placeholder="Enter Pincode" className="w-full bg-gray-50 border border-gray-200 rounded-full py-2 md:py-2.5 pl-4 md:pl-5 pr-10 md:pr-12 text-xs md:text-sm focus:outline-none focus:ring-1 focus:ring-red-200" />
                            <button className="absolute right-1 top-1 w-7 h-7 md:w-8 md:h-8 bg-black text-white rounded-full flex items-center justify-center hover:bg-gray-800 transition-colors">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                    {stores.map((store, idx) => (
                        <div key={idx} className="bg-white rounded-2xl p-5 md:p-6 border border-gray-100 shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:shadow-lg transition-all duration-300 group">
                            <div className="w-10 h-10 md:w-12 md:h-12 bg-red-50 rounded-xl flex items-center justify-center text-[#a51c30] mb-4 md:mb-6 group-hover:scale-110 transition-transform">
                            ...
                            <div className="flex items-center gap-2 text-gray-700 text-xs md:text-sm font-medium mb-4 md:mb-6">
                                <i className="ri-phone-fill text-[#a51c30]"></i> {store.phone}
                            </div>
                            <button onClick={() => openMap(store.mapEmbed, store.mapLink)} className="w-full border border-red-100 text-[#a51c30] py-2 md:py-2.5 rounded-xl text-xs md:text-sm font-bold hover:bg-red-50 transition-colors">
                                Get Directions
                            </button>
                        </div>
                    ))}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in" onClick={closeMap}>
                    <div className="bg-white w-full max-w-3xl rounded-2xl overflow-hidden shadow-2xl animate-scale-up" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-4 bg-gray-50 flex justify-end">
                            <a href={mapLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-[#a51c30] text-white px-5 py-2.5 rounded-lg text-sm font-bold hover:bg-[#8a1628] transition-colors">
