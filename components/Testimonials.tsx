...
export default function Testimonials() {
    const reviews = [
        { name: "Rahul Sharma", city: "Bangalore", text: "Simply superb laptop! Condition is like new only. Battery backup is also too good. Best place for refurbished tech in Bengaluru." },
        ...
        { name: "Meera Iyer", city: "Pune", text: "Got 70% off on HP Spectre. Display is stunning. Simtech Assured quality is real. Thank you for saving my money!" },
    ...
    return (
        <section className="py-10 md:py-16 bg-slate-50 border-t border-gray-200 overflow-hidden relative">
            <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: 'radial-gradient(#a51c30 1px, transparent 1px)', backgroundSize: '20px 20px' }}></div>
            ...
            <div className="max-w-6xl mx-auto px-4 mb-8 md:mb-12 text-center relative z-10">
                ...
                <div className="w-16 h-1 md:w-20 bg-[#a51c30] mx-auto rounded-full mb-4"></div>
                <p className="text-gray-500 text-xs md:text-base max-w-2xl mx-auto">Join our growing community of satisfied customers across India who have switched to premium refurbished tech.</p>
            </div>
            ...
            <div className="relative w-full overflow-hidden z-10">
                ...
                <div className="flex animate-marquee hover:pause whitespace-nowrap gap-4 md:gap-6 w-max py-4">
                    {[...reviews, ...reviews].map((review, idx) => (
                        <div key={idx} className="w-[280px] md:w-[380px] bg-white p-5 md:p-8 rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_50px_-10px_rgba(0,113,220,0.15)] border border-gray-100 hover:border-red-100 flex-shrink-0 whitespace-normal relative transition-all duration-300 hover:-translate-y-2 group cursor-default overflow-hidden">
                            <div className="absolute top-0 right-0 w-24 h-24 md:w-32 md:h-32 bg-gradient-to-br from-red-50 to-transparent rounded-bl-full -mr-8 -mt-8 transition-transform duration-500 group-hover:scale-110"></div>
                            <div className="absolute top-4 right-6 md:top-6 md:right-8 text-4xl md:text-5xl text-red-100 font-serif leading-none group-hover:text-red-200 transition-colors duration-300">&quot;</div>
                            ...
                            <div className="flex items-center gap-3 md:gap-4 mt-auto relative z-10 pt-4 md:pt-6 border-t border-gray-50">
                                <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-gradient-to-br from-[#a51c30] to-red-600 flex items-center justify-center text-white font-bold text-base md:text-lg shadow-lg shadow-red-200 ring-2 ring-white">
                                    {review.name.charAt(0)}
                                </div>
                                <div>
                                    <h4 className="font-bold text-gray-900 text-sm mb-0.5 group-hover:text-[#a51c30] transition-colors">{review.name}</h4>
                                ...
                                <div className="ml-auto bg-red-50 px-2 py-1 rounded-md">
                                    <i className="ri-verified-badge-fill text-[#a51c30] text-lg" title="Verified Buyer"></i>
                                </div>
                            </div>
                        </div>
                    ))}
